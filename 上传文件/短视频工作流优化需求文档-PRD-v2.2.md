...
graph TD
    ...
    %% HTTP节点 - 创建项目（混合策略优化）
    HTTP_PROJECT[hybrid_create_project<br/>🌐混合策略节点<br/>智能项目创建<br/>MCP优先，HTTP降级，状态感知]
    
    %% 并行分支起点（优化版）
    PARALLEL_START[parallel_branch_start<br/>🔀智能并行分支起点<br/>启动音频与视觉素材并行处理<br/>支持动态负载均衡与错误隔离]
    
    %% 音频处理分支（与视觉素材并行处理）
    subgraph AUDIO_BRANCH["🎵 音频处理分支（并行优化）"]
        IF_TTS{if_check_tts_enabled<br/>🔀条件判断节点<br/>检查是否启用TTS<br/>支持多路径并行启动}
        
        %% 并行音频处理子分支
        subgraph AUDIO_PARALLEL["🎵 音频并行处理子分支"]
            TOOL_TTS[tool_tts_synthesis<br/>🔧工具节点<br/>TTS语音合成<br/>音频为主时钟基准<br/>支持分段并行合成]
            CODE_AUDIO_PROCESS[code_audio_processing<br/>💻Code节点<br/>音频时长分析与字幕对齐<br/>实时音频特征提取]
            HTTP_SUBTITLE[http_generate_subtitle<br/>🌐HTTP请求节点<br/>生成字幕文件<br/>支持多语言并行生成]
            CODE_AUDIO_SYNC[code_audio_sync_point<br/>💻Code节点<br/>音频同步点检测<br/>为视觉素材提供时间锚点]
        end
        ...
        IF_TTS -->|同时启动| CODE_AUDIO_SYNC
        TOOL_TTS --> CODE_AUDIO_PROCESS
        TOOL_TTS --> CODE_AUDIO_SYNC
        CODE_AUDIO_PROCESS --> HTTP_SUBTITLE
    end
    
    %% 视觉素材处理分支（迭代并行优化版）
    subgraph VISUAL_BRANCH["🎬 视觉素材处理分支（迭代并行优化）"]
        %% 智能素材生成器（迭代节点优化）
        ITER_VISUAL[smart_asset_generator<br/>🔄智能素材生成器（迭代节点）<br/>并行模式：最大10个并发任务<br/>错误处理：移除错误输出并继续<br/>音频同步，实时追踪，批量处理]
        
        %% 迭代节点内部并行处理架构
        subgraph ITERATION_INTERNAL["🔄 迭代节点内部并行架构"]
            %% 按素材来源三分类（方案B架构）
            IF_MATERIAL_SOURCE{if_material_source_route<br/>🔀条件判断节点<br/>素材来源路由分类<br/>支持批量路由决策}
            
            %% AI生成素材分支（并行处理）
            subgraph AI_GENERATED["🤖 AI生成素材分支（并行）"]
                IF_AI_TYPE{if_ai_generation_type<br/>🔀条件判断节点<br/>AI生成类型选择<br/>支持混合类型并行}
                
                %% AI生成工具并行组
                subgraph AI_TOOLS_PARALLEL["🤖 AI工具并行组"]
                    TOOL_T2I[tool_doubao_text2image<br/>🔧工具节点<br/>文生图工具<br/>支持批量并行生成]
                    TOOL_T2V[tool_doubao_text2video<br/>🔧工具节点<br/>文生视频工具<br/>支持批量并行生成]
                    TOOL_I2V_BATCH[tool_doubao_image2video_batch<br/>🔧工具节点<br/>批量图转视频<br/>支持队列并行处理]
                end
            end
            
            %% 用户提供素材分支（并行处理）
            subgraph USER_PROVIDED["👤 用户提供素材分支（并行）"]
                IF_USER_TYPE{if_user_material_type<br/>🔀条件判断节点<br/>用户素材类型检查<br/>支持多类型并行处理}
                
                %% 用户素材并行上传组
                subgraph USER_UPLOAD_PARALLEL["👤 用户素材并行上传组"]
                    HTTP_USER_IMAGE[http_add_user_image<br/>🌐HTTP请求节点<br/>批量添加用户图片<br/>MCP优先登记，支持并行上传]
                    HTTP_USER_VIDEO[http_add_user_video<br/>🌐HTTP请求节点<br/>批量添加用户视频<br/>MCP优先登记，支持并行上传]
                    HTTP_USER_BGM[http_add_user_bgm<br/>🌐HTTP请求节点<br/>添加用户BGM<br/>MCP优先登记]
                end
            end
            
            %% BGM搜索插件分支（并行处理）
            subgraph BGM_SEARCH["🎵 BGM搜索插件分支（并行）"]
                TOOL_BGM_SEARCH[tool_bgm_search_plugin<br/>🔧工具节点<br/>BGM搜索插件<br/>关键词/风格/情绪并行搜索]
                HTTP_BGM_REGISTER[http_register_bgm_asset<br/>🌐HTTP请求节点<br/>批量登记BGM素材<br/>MCP优先登记，支持并行注册]
            end
            
            %% 素材质量检查与优化（并行处理）
            subgraph QUALITY_CHECK["🔍 素材质量检查（并行）"]
                CODE_QUALITY_CHECK[code_asset_quality_check<br/>💻Code节点<br/>并行素材质量检查<br/>分辨率、格式、时长验证]
                CODE_AUTO_ENHANCE[code_auto_enhance<br/>💻Code节点<br/>自动素材增强<br/>尺寸调整、格式转换]
            end
        end
        ...
        IF_AI_TYPE -->|图转视频| TOOL_I2V_BATCH
        TOOL_T2I --> CODE_QUALITY_CHECK
        TOOL_T2V --> CODE_QUALITY_CHECK
        TOOL_I2V_BATCH --> CODE_QUALITY_CHECK
        
        %% 用户提供分支并行路由
        IF_USER_TYPE -->|用户图片| HTTP_USER_IMAGE
        IF_USER_TYPE -->|用户视频| HTTP_USER_VIDEO
        IF_USER_TYPE -->|用户BGM| HTTP_USER_BGM
        HTTP_USER_IMAGE --> CODE_QUALITY_CHECK
        HTTP_USER_VIDEO --> CODE_QUALITY_CHECK
        ...
        %% 质量检查与增强路由
        CODE_QUALITY_CHECK --> CODE_AUTO_ENHANCE
        CODE_AUTO_ENHANCE --> CONVERGENCE
    end
...
🎵 **音频处理分支特性（并行优化）：**
- **智能并行分支起点**：`PARALLEL_START` 节点支持动态负载均衡与错误隔离
- **音频并行处理子分支**：包含分段并行合成、实时音频特征提取、多语言并行生成和音频同步点检测
- **条件分支**：TTS启用状态决定音频处理路径
- **时长分析**：自动分析音频时长，为视觉素材提供时间基准
- **字幕对齐**：音频与字幕精确同步，支持断句优化
- **音频策略**：实现音频为主时钟的对齐策略
- **错误恢复机制**：单个音频任务失败不影响其他并行任务
- **实时进度追踪**：提供详细的音频处理进度信息

🎬 **视觉素材分支特性（迭代并行优化）：**
- **智能素材生成器（迭代节点）**：采用 Dify 迭代节点架构，核心特性包括：
  - **并行模式配置**：最大10个并发执行，动态并行策略，启用负载均衡
  - **错误处理优化**：移除错误输出并继续，最大错误容忍数3，错误隔离防止影响其他任务
  - **批量处理能力**：支持文生图（批量4张）、文生视频（并发3个）、图转视频（并发4个）的批量并行生成
  - **质量检查与增强**：并行素材质量检查（分辨率、格式、时长、文件大小），自动增强处理
  - **智能重试机制**：单项最大重试2次，1秒延迟，指数退避策略
  - **进度追踪配置**：5秒间隔进度回调，详细日志记录，实时性能监控
  - **资源优化**：批量处理、资源池化、自动内存管理，单项30秒超时
- **迭代工作流内部架构**：
  - **素材来源路由**：支持AI生成、用户提供、BGM搜索三种来源的批量路由决策
  - **AI生成工具并行组**：文生图、文生视频、图转视频工具支持队列管理和并发控制
  - **用户素材并行上传组**：支持图片、视频、BGM的批量并行上传，MCP优先登记
  - **BGM搜索并行处理**：关键词、风格、情绪并行搜索，批量登记BGM素材
  - **质量检查并行处理**：10个并发的素材质量检查，自动增强和格式转换
...
- **if-else节点**：条件判断和流程分支
- **iteration节点（优化配置）**：循环处理多个素材，支持：
  - **并行执行控制**：最大并发数10，动态负载均衡，智能队列管理
  - **错误恢复策略**：错误隔离、智能重试、降级处理、断点续传
  - **性能监控指标**：迭代时长、并行执行数、错误率、吞吐量、资源利用率
  - **输出聚合方式**：合并数组模式，标准化输出格式（id、type、url、duration、quality_score等）
- **并行分支节点（优化配置）**：启动多个并行处理分支，支持：
  - **动态负载均衡**：根据系统资源自动调整并发数
  - **错误隔离机制**：单个分支失败不影响其他并行分支
  - **资源池化管理**：复用连接和计算资源，提升处理效率
  - **实时监控反馈**：提供友好的进度展示和错误提示
- **收敛节点**：等待并行分支完成，同步结果
- **end节点**：返回最终结果
...
### 5.2.1 Dify迭代节点配置规范（优化版）

#### 5.2.1.1 迭代节点核心配置

**smart_asset_generator（智能素材生成器）迭代节点配置：**

```yaml
iteration_node:
  node_id: "smart_asset_generator"
  node_type: "iteration"
  title: "智能素材生成器（迭代节点）"
  
  # 迭代基础配置
  iteration_config:
    input_variable: "storyboard_segments"  # 输入变量：分镜段落数组
    output_variable: "generated_assets"    # 输出变量：生成的素材数组
    
    # 并行模式配置（Dify高级功能）
    parallel_mode:
      enabled: true                        # 启用并行模式
      max_parallel_executions: 10          # 最大并发执行数
      parallel_strategy: "dynamic"         # 动态并行策略
      load_balancing: true                 # 启用负载均衡
      
    # 错误处理配置（Dify高级功能）
    error_handling:
      error_response_method: "remove_error_outputs_and_continue"  # 移除错误输出并继续
      max_error_tolerance: 3               # 最大错误容忍数
      error_isolation: true                # 错误隔离，防止影响其他并行任务
      retry_failed_items: true             # 重试失败项目
      retry_config:
        max_retries: 2                     # 单项最大重试次数
    ...
    progress_tracking:
      enabled: true                        # 启用进度追踪
      progress_callback_interval: 5        # 进度回调间隔（秒）
    ...
    performance_optimization:
      ...
      memory_management: "auto"            # 自动内存管理
      timeout_per_item: 30                 # 单项超时时间（秒）
      
  # 迭代工作流配置
  iteration_workflow:
    start_node: "material_source_router"
    
    nodes:
      # 素材来源路由节点
      - node_id: "material_source_router"
        node_type: "if"
        title: "素材来源路由"
        conditions:
          - condition: "{{#item.material_source}}ai_generated{{/item.material_source}}"
            logical_operator: "and"
            next_node: "ai_generation_router"
          - condition: "{{#item.material_source}}user_provided{{/item.material_source}}"
            logical_operator: "and"
            next_node: "user_material_processor"
          - condition: "{{#item.material_source}}bgm_search{{/item.material_source}}"
            logical_operator: "and"
            next_node: "bgm_search_processor"
            
      # AI生成路由节点
      - node_id: "ai_generation_router"
        node_type: "if"
        title: "AI生成类型路由"
        conditions:
          - condition: "{{#item.generation_type}}text_to_image{{/item.generation_type}}"
            logical_operator: "and"
            next_node: "text_to_image_generator"
          - condition: "{{#item.generation_type}}text_to_video{{/item.generation_type}}"
            logical_operator: "and"
            next_node: "text_to_video_generator"
          - condition: "{{#item.generation_type}}image_to_video{{/item.generation_type}}"
            logical_operator: "and"
            next_node: "image_to_video_generator"
            
      # AI生成工具节点（并行处理）
      - node_id: "text_to_image_generator"
        node_type: "tool"
        title: "文生图工具（批量并行）"
        tool_name: "doubao_text2image"
        tool_parameters:
        ...
        parallel_config:
          ...
          queue_management: true           # 队列管理
          
      - node_id: "text_to_video_generator"
        node_type: "tool"
        title: "文生视频工具（批量并行）"
        tool_name: "doubao_text2video"
        tool_parameters:
          prompt: "{{#item.visual_prompt}}"
          ...
          resolution: "1080x1920"
        parallel_config:
          max_concurrent: 3                # 视频生成并发限制
          resource_intensive: true         # 资源密集型任务
          
      - node_id: "image_to_video_generator"
        node_type: "tool"
        title: "图转视频工具（批量并行）"
        tool_name: "doubao_image2video"
        tool_parameters:
      ...
      # 用户素材处理节点
      - node_id: "user_material_processor"
        node_type: "http-request"
        title: "用户素材处理器（批量并行）"
        http_config:
          method: "POST"
          url: "{{#sys.env.CAPCUT_API_BASE}}/api/v1/materials/batch_upload"
          headers:
            Authorization: "Bearer {{#sys.env.CAPCUT_TOKEN}}"
            Content-Type: "application/json"
          body:
            materials: "{{#item.user_materials}}"
            project_id: "{{#sys.project_id}}"
        parallel_config:
          max_concurrent: 8                # 上传并发限制
          retry_on_failure: true
          
      # BGM搜索处理节点
      - node_id: "bgm_search_processor"
        node_type: "tool"
        title: "BGM搜索处理器"
        tool_name: "bgm_search_plugin"
        tool_parameters:
          keywords: "{{#item.bgm_keywords}}"
          style: "{{#item.bgm_style}}"
          mood: "{{#item.bgm_mood}}"
          duration: "{{#item.duration}}"
      ...
      - node_id: "quality_checker"
        ...
        code: |
          ...
          def check_asset_quality(asset):
              """并行检查单个素材质量"""
              quality_score = 0
              issues = []
              
              # 检查分辨率
              if asset.get('width', 0) >= 1080 and asset.get('height', 0) >= 1920:
                  quality_score += 30
              else:
                  issues.append("分辨率不足")
                  
              # 检查格式
              if asset.get('format', '').lower() in ['mp4', 'mov', 'jpg', 'png']:
                  quality_score += 25
              else:
                  issues.append("格式不支持")
          ...
          def main():
              ...
              with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                  quality_results = list(executor.map(check_asset_quality, assets))
              
              # 过滤通过质量检查的素材
              passed_assets = [
                  asset for asset, result in zip(assets, quality_results)
                  if result['passed']
              ]
              
              return {
                  'checked_assets': passed_assets,
                  'quality_results': quality_results,
                  'pass_rate': len(passed_assets) / len(assets) if assets else 0
              }
              
        parallel_config:
          max_concurrent: 10               # 质量检查并发数
          
      # 素材增强节点
      - node_id: "asset_enhancer"
        node_type: "code"
        title: "素材自动增强器"
        code: |
          def enhance_assets(assets):
              """自动增强素材"""
              enhanced_assets = []
              ...
              for asset in assets:
                  ...
                  # 自动调整尺寸
                  if asset.get('width', 0) < 1080 or asset.get('height', 0) < 1920:
                      enhanced_asset['needs_resize'] = True
                      enhanced_asset['target_resolution'] = '1080x1920'
                      
                  # 格式转换
                  if asset.get('format', '').lower() not in ['mp4', 'jpg', 'png']:
                      enhanced_asset['needs_conversion'] = True
                      enhanced_asset['target_format'] = 'mp4' if asset.get('type') == 'video' else 'jpg'
                      
                  # 添加增强标记
                  enhanced_asset['enhanced'] = True
                  enhanced_asset['enhancement_timestamp'] = "{{#sys.timestamp}}"
                  
                  enhanced_assets.append(enhanced_asset)
                  
              return {'enhanced_assets': enhanced_assets}
              
    # 迭代结束条件
    end_conditions:
      - condition: "all_items_processed"   # 所有项目处理完成
      - condition: "max_iterations_reached" # 达到最大迭代次数
      - condition: "error_threshold_exceeded" # 错误阈值超出
      
  # 输出聚合配置
  output_aggregation:
    aggregation_method: "merge_arrays"     # 合并数组
    output_schema:
      generated_assets:
        type: "array"
        items:
...
**并行处理优化：**
- **动态负载均衡**：根据系统资源自动调整并发数
- **智能队列管理**：优先处理小文件，延后处理大文件
- **资源池化**：复用连接和计算资源
- **内存管理**：自动清理临时文件和缓存

**错误恢复机制：**
- **错误隔离**：单个任务失败不影响其他并行任务
- **智能重试**：根据错误类型选择重试策略
- **降级处理**：关键路径失败时自动降级到备用方案
- **断点续传**：支持从失败点继续处理

**进度追踪与监控：**
- **实时进度**：提供详细的处理进度信息
- **性能指标**：监控吞吐量、延迟、错误率等关键指标
