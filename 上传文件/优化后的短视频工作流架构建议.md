...
graph TD
    ...
    subgraph AUDIO_BRANCH [" 🎵 音频处理分支（迭代优化版） "]
        direction TB
        AUDIO_ITER([🔄 ITERATION节点<br/>iteration_audio_segments<br/>音频片段并行处理<br/>最大并发数: 6])
        ...
        AUDIO_SYNC([💻 CODE节点<br/>code_audio_sync<br/>音频同步收敛<br/>时长基准确定])
        
        AUDIO_ITER --> TTS_MULTI
        TTS_MULTI --> AUDIO_ENHANCE
        AUDIO_ENHANCE --> SUBTITLE_ALIGN
        SUBTITLE_ALIGN --> AUDIO_FEATURES
        AUDIO_FEATURES --> AUDIO_SYNC
    end
    
    %% 视觉素材分支（优化迭代调度）
    subgraph VISUAL_BRANCH [" 🎬 视觉素材分支（智能调度优化版） "]
        direction TB
        VISUAL_ITER([🔄 ITERATION节点<br/>iteration_visual_smart<br/>智能素材生成器<br/>最大并发数: 8<br/>智能调度: 启用])
        SMART_ROUTE{🔀 智能路由<br/>优先级调度<br/>资源感知分配}
        T2I_POOL[🔧 TOOL节点<br/>tool_t2i_pool<br/>文生图资源池<br/>优先级: 高]
        I2V_POOL[🔧 TOOL节点<br/>tool_i2v_pool<br/>图生视频资源池<br/>优先级: 中]
        T2V_POOL[🔧 TOOL节点<br/>tool_t2v_pool<br/>文生视频资源池<br/>优先级: 低]
        QUALITY_CHECK[💻 CODE节点<br/>code_quality_realtime<br/>实时质量评分<br/>自动重生成机制]
        VISUAL_OUTPUT([📦 视觉素材输出<br/>标准化格式<br/>质量保证])
        
        VISUAL_ITER --> SMART_ROUTE
        SMART_ROUTE --> T2I_POOL
        SMART_ROUTE --> I2V_POOL
        SMART_ROUTE --> T2V_POOL
        T2I_POOL --> QUALITY_CHECK
        I2V_POOL --> QUALITY_CHECK
        T2V_POOL --> QUALITY_CHECK
        QUALITY_CHECK --> VISUAL_OUTPUT
    end
    ...
    class LLM llmNode
    class PARSE,AUDIO_ENHANCE,SUBTITLE_ALIGN,AUDIO_FEATURES,AUDIO_SYNC,QUALITY_CHECK,BGM_SMART_RANK,MATERIAL_CHECK,TIMELINE_CHECK,FORMAT_CHECK,PARAM_CHECK,COPYRIGHT_CHECK,AUDIO_STRATEGY codeNode
    class HTTP_PROJECT,HTTP_ASSETS,HTTP_RENDER,BGM_MIX,TIMELINE,RENDER httpNode
    class TTS_MULTI,T2I_POOL,I2V_POOL,T2V_POOL,KEYWORD_SEARCH,STYLE_SEARCH,EMOTION_SEARCH,BEAT_SEARCH toolNode
